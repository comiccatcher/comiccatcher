...
class BrowserView(ft.Column):
    def __init__(self, page: ft.Page, config_manager: ConfigManager, on_open_detail, on_update_header, on_load_complete, on_navigate):
        ...
        self.main_content = ft.ListView(expand=1, spacing=15, padding=ft.padding.only(bottom=40, left=10, right=10))
        self.main_content.controls.append(ft.Text("Initializing Browser...", size=12, color=COLORS.GREY_700))
        
        self.loading_indicator = ft.ProgressRing(visible=False, width=30, height=30)
        self.loading_container = ft.Container(content=self.loading_indicator, alignment=ft.alignment.center, visible=False)
    ...
    async def load_feed(self, url: str, title: str = None, force_refresh: bool = False):
        ...
        try:
            ...
            if self.is_dashboard:
                logger.debug("Entering Dashboard mode")
                ...
                logger.debug(f"Entering List mode (pub_mode={self.is_pub_mode}, method={method})")
                self.main_content.controls.clear()
                if self.is_pub_mode:
    ...
    def _render_dashboard(self, feed: OPDSFeed):
        try:
            logger.debug(f"Building dashboard with {len(feed.groups or [])} groups and {len(feed.publications or [])} loose publications")
            self.main_content.controls.append(ft.Text(f"DEBUG: Dashboard {feed.metadata.title}", size=10, color=COLORS.GREY_700))
            ...
            if feed.navigation:
                ...
                if nav_items:
                    logger.debug(f"Adding {len(nav_items)} nav items")
            ...
            if feed.groups:
                for i, group in enumerate(feed.groups):
                    group_title = group.metadata.title if (hasattr(group, 'metadata') and group.metadata) else f"Group {i}"
                    logger.debug(f"Processing group: {group_title}")
                    ...
                    if group.publications:
                        logger.debug(f"Adding carousel with {len(group.publications)} publications")
            ...
            if feed.publications:
                logger.debug(f"Adding {len(feed.publications)} loose publications")
            ...
            self.main_content.controls.append(ft.Text(f"Render Error: {e}", color=COLORS.ERROR))
    ...
    def create_pub_card(self, pub: Publication, is_carousel=False, token: Optional[int] = None):
        ...
        img = ft.Image(src=TRANSPARENT_DATA_URL, fit=ft.ImageFit.COVER, width=140, height=200)
