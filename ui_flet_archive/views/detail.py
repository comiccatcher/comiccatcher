...
class DetailView(ft.Column):
    ...
    def _render_details(self, pub: Publication, base_url: str, manifest_url: str):
        ...
        img_control = ft.Image(src=TRANSPARENT_DATA_URL, width=300, fit=ft.ImageFit.CONTAIN, border_radius=10)
        if image_url:
    ...
    async def _load_carousel_data(self, url, row):
        try:
            ...
            for pub in pubs[:15]:
                ...
                mini_img = ft.Image(src=TRANSPARENT_DATA_URL, height=160, fit=ft.ImageFit.CONTAIN, border_radius=5)
