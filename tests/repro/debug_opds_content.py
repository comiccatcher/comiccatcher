...
def get_reconciliation_report(page: FeedPage, obfuscate: bool = False) -> dict:
    ...
    if main and main.total_items is not None and page.pagination_template:
        scroll_mode = "Virtualized Grid (pre-allocates rows for total count)"
    elif main and page.next_url:
...
async def main():
    ...
    parser.add_argument("--raw", action="store_true", help="Dump raw response")
    ...
    parser.add_argument("--save", help="Save raw response and reconciliation report JSON to tests/feeds/ using this ID")
    ...
    async with APIClient(profile) as api:
        try:
            ...
            is_xml = "xml" in content_type or "atom" in content_type or (not "json" in content_type and ("<feed" in resp.text or "<?xml" in resp.text))

            if is_xml:
                console.print(f"[bold green]Detected OPDS 1.2 XML feed.[/bold green]")
                feed = await parse_opds12(resp.text, api, args.url)
                raw_data = None 
            else:
            ...
            if args.save:
                ...
                raw_path = os.path.join(raw_dir, f"{args.save}{ext}")
                ...
                console.print(f"[bold green]Saved raw response to:[/bold green] {raw_path}")
            
            if args.raw:
                if is_xml:
                    console.print(f"\n[bold magenta]--- Raw Response (XML) ---[/bold magenta]")
                    syntax = Syntax(resp.text, "xml", theme="monokai", line_numbers=True)
                    console.print(syntax)
                else:
                    console.print(f"\n[bold magenta]--- Raw Response (JSON) ---[/bold magenta]")
            ...
            if not is_manifest and not is_xml:
                # 1. Check Content-Type
                if "webpub" in content_type or "divina" in content_type:
                    is_manifest = True
            ...
            if is_manifest:
                ...
                console.print(f"[bold green]Feed object ready.[/bold green]")
                page = FeedReconciler.reconcile(feed, args.url)
                ...
                if args.json:
                    ...
                    if report['next_url']: console.print(f"Next URL:     {report['next_url']}")
                    ...
                    for i, section in enumerate(page.sections):
                        ...
                        for item in section.items:
                            ...
                            if args.verbose:
                                links = []
                                if item.raw_link: links.append(f"[dim]href: {item.raw_link.href}[/dim]")
                                if item.raw_pub:
                                    for l in (item.raw_pub.links or []):
                                        r = l.rel or "none"
                                        links.append(f"[dim]{r}: {l.href}[/dim]")
                                if item.download_url: links.append(f"[green]acq: {item.download_url}[/green]")
                                table.add_row(type_str, item.title, str(item.identifier)[:40] + "...", " ".join(details) + "\n" + "\n".join(links))
                            else:
                                table.add_row(type_str, item.title, str(item.identifier)[:40] + "...", " ".join(details))
                        console.print(table)
                        meta = []
                        ...
                        if meta: console.print(f"   [dim cyan]{' | '.join(meta)}[/dim cyan]")
                        
                    if page.facets:
                        console.print(f"\n[bold blue]Facets Found:[/bold blue] {len(page.facets)}")
                        for f in page.facets:
                            title = getattr(f, "metadata", None)
                            title = getattr(title, "title", "Untitled Facet") if title else "Untitled Facet"
                ...
                if args.save and page:
                    ...
                    with open(exp_path, "w") as f: json.dump(report, f, indent=2)
                    console.print(f"[bold green]Saved reconciliation report to:[/bold green] {exp_path}")
